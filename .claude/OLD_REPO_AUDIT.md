...
| ~~B3~~ | ~~Video render pipeline~~ **DONE 2026-07-12** → `VideoService`: dual path, poll in the 5-min cron (not inline 30×30s), download → R2 (no Mux), public `/api/v/:token` stream. R2 binding `VIDEOS` in api wrangler. | `apps/worker/src/trigger/render-video.task.ts`, `test-video.task.ts` | — |
| ~~B4~~ | ~~Resend webhooks~~ **DONE 2026-07-12** → `WebhookService`: **inline HMAC-SHA256** (not svix — no new dep), event→EmailEvent, P2002 idempotency, bounce/complaint/unsub → `suppressContact`. | `apps/api/src/webhooks/resend.controller.ts` | — |
| ~~B4~~ | ~~Send rate limiting~~ **DONE 2026-07-12** → `CampaignService.runSendTick`: budget = maxSendsPerHour/Day counted over a time window; sequential sends per tick. | `apps/worker/src/trigger/send-outreach.task.ts` | — |
| ~~B4~~ | ~~Template `{{var}}` rendering~~ **DONE 2026-07-12** → shared `apps/api/src/lib/template-render.ts` (same two regexes as the frontend). Terminology overrides + injection scrub deliberately **not** ported (out of scope). | `packages/workflows/src/index.ts` | — |
